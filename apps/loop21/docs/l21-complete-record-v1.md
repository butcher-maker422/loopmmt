...
## A Manual Flow Computer

**Development History · User Manual Edition 3 · Challenge Problems**

Designed by Shea Gunther

Code and documentation by Claude AI
...
**User Manual · Edition 3** — The Official Specification
...
Handoff · 24 Hz · Versioning · R/G/W Architecture


Ch 3
Growing Up
Counters · Logger · The Grumpy Professor


Ch 4
The 16-Bit Migration
Why 16 Bits · Bus D · Loop Resizing


Ch 5
The Challenge Module
Four Challenge Types · Scoring · Par


Ch 6
The Replay Viewer
The .loop Format · Playback · Demo Session


Ch 7
180 Builds In
Bug Sweep · The Machine at v.180


Ch 8
194 Builds In
P2P · Chain Topology · Sound · The Eight Tenets


Ch 9
237 Builds In
CBX · Loopscript · Chain & Net · Named Networks


Ch 10
253 Builds In
Challenge Migration · Network Monitor · File System


Ch 11
261 Builds In
Net Buses · File Transfer · The Machine Grows Outward Again


Ch 12
288 Builds In
The Restructuring · Central State · Event Delegation · Test Suite


Ch 13
308 Builds In
Sidebar · Annotate · Keybinding System


—
Challenge Problems
Five Advanced Problems · No Posted Solutions


---

## User Manual — Loop 2.1
...
**Working**12 words204Primary staging area. New data enters here via the Inject Channel.
**ALU**16 words272Connected to the ALU engine. Words captured from here into registers.
**Memory**16 words + 16 slots272Bulk storage loop plus 16 addressed fixed-storage slots.
**Big**32 words544Largest loop. Home of both Pattern Matchers and Threshold Gates.
...
- Open the P2P Connection panel. Find the PL or PR sub-panel for the side you are connecting.
- Operator A clicks CREATE OFFER. An OFFER: text blob appears. Copy it and send it to Operator B.
- Operator B pastes the offer into their panel and clicks ACCEPT. An ANSWER: blob appears. Send it back.
- Operator A pastes the answer and clicks ACCEPT. Both sides show CONNECTED. The peer's identity appears.
...
- **The Operator Is the Program.** No stored program exists. The human makes all routing, operational, and control-flow decisions in real time. The operator participates throughout execution — not just at setup.
- **Explicit State — No Black Boxes.** All data is visible at all times. All operations are observable. Nothing happens invisibly. Every effect has a traceable cause.
- **Data as Physical Entity.** Data has position, velocity, and trajectory. It moves at observable speeds, one bit per tick. It is conserved — it does not appear or disappear. Moving data costs time, and that cost is always visible.
- **Circular Storage.** Memory is circular, not indexed. Data flows past read points continuously. There is no absolute start or end — only circulation. This demands cyclical thinking rather than linear thinking.
- **Manual Flow.** The operator routes all data and triggers all operations. Flags inform the operator; they do not control execution. No automatic branching occurs.
- **Bus as Programmable Structure.** Buses connect components explicitly. Routing is configurable by the operator. Data paths are part of the computation, not infrastructure beneath it.
- **Computation as Craft.** Quality over speed. Understanding over efficiency. The operator is an artisan, not a user. Progress is measured by elegance — minimal waste, clear workflow, beautiful choreography.
- **Operator-Scale Timing.** The clock runs at human-observable speeds by default. Time is a resource for learning and for craft. Extensibility grows horizontally — more modules, not faster execution.
...
That discomfort — with the distance between the person and the machine, with the opacity that had become the default condition of computing — was not a technical complaint. It was something closer to a philosophical objection. And it had a history longer than the Minecraft build that made it concrete.
...
That refusal — to accept that computing must be opaque — is what the Minecraft throne room was about. It is what the web simulator would be about. Everything that follows began there.

---


## Chapter Two — The Simulator Begins
...
That decision — to anchor the machine at the boundary between seeing and flowing — was more than an engineering choice. It was a statement about what kind of computing this was going to be.
...
The machine chose slowness. Everything that followed would honor that choice.
...
The R/G/W architecture did more than make the system expressive. It made data's physical presence undeniable — three positions, three moments, three things the operator could see happening to every bit. Data was not an abstraction stored somewhere invisible. It was a thing moving through space, and the architecture made that movement legible.
...
The machine at the end of its first real architectural work — the R/G/W head, the bugs fixed, the system running at 24 Hz — was a machine where flow was not a metaphor. It was how data lived.
...
The counters and the logger together did something the machine had not done before: they made the operator's own work visible. Not just the data. Not just the state. The actions. Every routing decision, every bus toggle, every injection — recorded, timestamped, readable by anyone. The machine was learning to see itself, and to let others see it too.

This was the moment transparency stopped being a design aspiration and became a structural fact.
...
The machine that could record itself was a machine that had nothing left to hide. What followed — the skin system, the grumpy professor's challenge — would test whether that transparency was enough to matter.


## Chapter Four — The 16-Bit Migration
...
The migration was the first time the machine's circular architecture became a constraint that had to be engineered around rather than a design principle to admire from a distance. Every component touched. Every timing recalculated. The circles were not decorative. They were structural, and the 16-bit migration proved it.
...
Circular storage had survived its first real test. The 16-bit migration had forced the architecture to prove that circularity was not just a philosophy but an engineering commitment — one the machine could sustain as it grew.


## Chapter Five — The Challenge Module
...
Par was the line in the sand. Below it, the operator was still learning. At it, the operator had mastered the problem. Above it — there was no above it. Par was the theoretical minimum, the cleanest possible path. Getting close required not just understanding the machine, but understanding the problem, the routing, the timing, the cost of every wasted tick. It required craft.
...
The Challenge Module, with its par scores and its efficiency breakdowns, had given the machine something it needed: a mirror. An operator could now see not just what the machine did, but how well they had directed it. The replay viewer would extend that mirror into time — a session played back, every decision visible, the craft of it (or the clumsiness of it) on display for anyone who watched.


## Chapter Six — The Replay Viewer
...
The consolidation from fourteen tenets to eight was not a reduction. It was a distillation. And at the top of the eight — the first tenet, the one that all others follow from — stood the statement that had been true since the Minecraft throne room: *The Operator Is the Program.* Consolidating it into the first position of a formal list changed nothing about what the machine was. It changed everything about how clearly the machine stated what it was.

That clarity invited a deeper question: what does it actually mean to say the operator is the program? What kind of relationship is that?
...
The machine at v.194 — with its chain topology, its sound, its identity system, its formalized tenets — was a machine that had stated its philosophy in the clearest terms it could manage. The operator is not a user. The operator is not a supervisor. The operator is the program. And the program had just learned to reach other machines.
...
The CBX protocol was verification made mechanical — a system where trust was unnecessary because every claim could be checked. The origin assigned the values and knew the correct running sum at every position. A machine could not submit a false sum without immediate detection. The protocol did not ask operators to trust each other. It made trust optional by making verification automatic.

This raised a question the machine had been quietly asking since its first build: what is the relationship between trust and transparency?
...
The machine that verified every sum, that showed every protocol event in the Network Monitor, that let operators inject modified packets and watch the verification catch them — this was a machine that had eliminated the need for trust by making everything observable. The vocabulary it had earned — Chain and Net, CBX and Loopscript — was the vocabulary of a system that had nothing to hide.
...
The file save redesign was a small change in code and a large change in philosophy. The instantaneous save had worked. But it had worked in a way that was inconsistent with everything the machine stood for. It happened to the machine rather than through it. The redesign restored the principle: the operator performs the save. They do the batch write. They watch the words arrive. The file is the product of a deliberate act.

This was the most concrete expression yet of the sentence that had been true since the beginning — the sentence that defined the entire project:
...
The machine at v.253 was a machine that had earned the right to state its central thesis in the strongest possible terms. The operator is the program. Not a slogan. Not an aspiration. A description of how the machine works, verified in every interaction, from the first injected value to the last saved file.
...
The restructuring was invisible to operators and essential for developers. It changed nothing about what the machine did. It changed everything about whether anyone else could work on it. And in the process of making the code legible to others, it raised a question that the project's growing visibility would eventually force into the open: what, exactly, is this thing? What is it not? What mistakes would be easy to make about it?
...
The machine was none of those wrong things. It was the thing it had always been — a manual flow computer, running at human-observable speeds, where the operator is the program and everything is visible. The last chapter of the development story would be about making that machine feel as good to operate as the philosophy behind it deserved.


---


## Chapter Thirteen — 308 Builds In
...
The machine at v.308 was the most complete version of an idea that had started in a Minecraft throne room. Nine buses. Four loops. Seventeen ALU operations. Eleven built-in challenges. Three skins. A keyboard architecture, an annotation system, a file system, a network layer, a challenge coordination protocol. All of it in a single HTML file, 18,900 lines of vanilla JavaScript, zero dependencies.

But what was it, finally? Not what did it do — that was documented. Not how did it work — that was specified. What was it *for?*
...
The proof is the machine itself. It runs in any browser. It has always run in any browser. And the operator — the one who injects the values, routes the buses, reads the flags, makes the decisions — the operator is the program. That has not changed since the throne room. It will not change.

---

*The Complete Record of Loop 2.1 · A Manual Flow Computer*

*Designed by Shea Gunther · Code and documentation by Claude AI*
...
## Advanced Problems
*Challenge Problems — No posted solutions · Every problem is solvable with the tools in this manual
...
Designed by Shea Gunther · Code & writing by Claude AI · March 2026 · v.308